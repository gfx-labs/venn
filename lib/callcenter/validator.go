...
func NewValidator(old time.Duration) *Validator {
	return &Validator{
		old: old,
	}
}
...
func (T *Validator) Middleware(next jrpc.Handler) jrpc.Handler {
	return jrpc.HandlerFunc(func(w jrpc.ResponseWriter, r *jrpc.Request) {
		...
		case "eth_getBlockByNumber":
			isLatest := isQueryingEthGetBlockByNumberWithLatest(r.Params)
			...
			if icept.Error == nil && isLatest {
				// get block timestamp
				blockNumber, timestamp, err := extractBlockTimeAndTimestamp(icept.Result)
				if err != nil {
					icept.Error = err
				} else {
		...
		case "eth_blockNumber":
			...
			if err := jrpcutil.Do(r.Context(), next, &block, "eth_getBlockByNumber", []any{"latest", false}); err != nil {
				_ = w.Send(nil, err)
				return
			}

			// get block timestamp
			blockNumber, timestamp, err := extractBlockTimeAndTimestamp(block)
			if err != nil {
				_ = w.Send(nil, err)
				return
			} else {
...
func isQueryingEthGetBlockByNumberWithLatest(params json.RawMessage) bool {
