...
type Forgers = util.Multichain[*Forger]
...
func New(p Params) (r Result, err error) {
	r.Forgers, err = util.MakeMultichain(
		p.Chains,
		func(chain *config.Chain) (*Forger, error) {
			if !chain.ForgeBlockReceipts {
				return nil, nil
			}

			remote, err := util.ChooseChain3(
				chain.Name,
				p.Stalkers,
				p.Cachers,
				p.Clusters,
...
func (T *Forger) ServeRPC(w jrpc.ResponseWriter, r *jrpc.Request) {
	switch r.Method {
	case "eth_getBlockReceipts":
		...
		wg.Go(func() error {
			return jrpcutil.Do(r.Context(), T.remote, &rawLogs, "eth_getLogs", []any{
		...
		wg.Go(func() error {
			return jrpcutil.Do(r.Context(), T.remote, &block, "eth_getBlockByNumber", []any{
				blockNumber[0],
				true,
			})
		})
